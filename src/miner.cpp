...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ITISMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ITISMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ITISMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("itis-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("ITISMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running ITISMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadITISMiner() exception");
    } catch (...) {
        LogPrintf("ThreadITISMiner() exception");
    }

    LogPrintf("ThreadITISMiner exiting\n");
}
